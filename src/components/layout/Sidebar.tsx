  ...
  return (
    ...
    >
          ...
          {navItems.map((item) => (
            <li key={item.name} className="mb-2 px-4">
              <Link
                to={item.path}
                className={cn(
                  "flex items-center px-4 py-3 text-gray-600 rounded-lg hover:bg-attendo-50 hover:text-attendo-500",
                  location.pathname === item.path && "bg-attendo-50 text-attendo-500"
                )}
              >
      ...
      <div className="p-4 border-t border-gray-200">
        <div className="flex items-center">
          <div className="h-8 w-8 rounded-full bg-attendo-200 flex items-center justify-center text-attendo-700 font-medium">
            HR
          </div>
          {!collapsed && (
            <div className="ml-3">
